...
To keep the notation readable, fix one feature \(d\). For each Monte Carlo
draw \(s\), let \(y_{ds}\in\mathbb{R}^N\) be the vector of latent
log-abundances across the \(N\) samples for that feature and draw. In the
formulas below, I write \(y_s\) instead of \(y_{ds}\) because the feature is
already fixed; the sample index \(n\) is the coordinate inside that
\(N\)-vector, so it does not appear separately.

If you prefer a fully indexed view, think of the data as \(y_{dns}\), where
\(n\) indexes samples, \(d\) indexes features, and \(s\) indexes Monte Carlo
draws. This section fixes \(d\) and then works with the length-\(N\) vector
\(y_{ds} = (y_{d1s}, \dots, y_{dNs})^T\) for each draw \(s\).
...
Here \(X\) is the fixed-effects design matrix and \(Z\) is the random-effects
design matrix for the \(N\) samples. The vector \(\beta_s\) contains the
draw-specific fixed effects, and \(\theta_s\) denotes the covariance
parameters for that draw. As in `lme4`, BLMM uses a relative covariance
parameterisation in which \(\Sigma_{\theta_s}\) is encoded through a
lower-triangular factor \(\Lambda_{\theta_s}\).
...
BLMM uses the profiled `lme4`-style Gaussian LMM formulation rather than a
dense \(N \times N\) covariance-matrix formulation. That is not a different
statistical model; it is the algebraic form used to evaluate the same REML
criterion efficiently. The profiled form is convenient because it lets us
profile out the fixed effects and residual scale first, then work with smaller
matrices that depend on the covariance parameters.

After profiling out the fixed effects and residual scale, the objective for a
draw depends only on the covariance parameters. In the profiled formulation
used here, the key matrices are
...
where \(L_{\theta_s}\) is the Cholesky factor of \(A_{\theta_s}\). These
matrices depend on \(X\), \(Z\), and \(\theta_s\), but not on the response
vector itself. The remaining response-dependent quantity for draw \(s\) is the
profiled weighted residual sum of squares,
...
and \(L_{X,\theta_s}\) is the Cholesky factor of \(M_{\theta_s}\). Intuitively,
\(R_{ZY,\theta_s}\) is the random-effects projection of the response and
\(C_{\beta,\theta_s}\) is the profiled fixed-effects solve. Up to constants, the
profiled REML criterion for one draw is
...
For one feature, the full Monte Carlo response block is
\(Y_d = [y_{d1}, \dots, y_{dS}] \in \mathbb{R}^{N \times S}\), where column
\(s\) is the response vector \(y_{ds}\) for draw \(s\). The batched anchor
evaluation reuses structure shared across those \(S\) columns:
...
5. one average of the per-draw profiled residual log terms across draws.

That batching step is the core computational advantage. BLMM does **not** loop
over \(S\) separate REML optimisations inside the anchor optimiser. It factors
the shared pieces once per feature, then reuses them across draws.

More concretely, the anchor fit targets the average of the per-draw profiled
REML criteria. For a fixed feature, the determinant terms depend only on \(X\),
\(Z\), and the covariance parameters, so they are shared across draws. The only
draw-specific term that must be averaged is the profiled residual log term:
...
+ \frac{N - p}{S}\sum_{s=1}^S
 \log\left(\frac{\mathrm{PWRSS}_s(\phi)}{N - p}\right)
\right\},
\]

Here \(\phi\) is the unconstrained optimisation parameter, and positive
components of \(\theta\) are represented on the log scale. The draw index \(s\)
is the Monte Carlo replicate; the sample index is already inside \(y_{ds}\).

This is the same profiled `lme4`-style formulation used by the code. The anchor
fit, the draw-specific correction, and the curvature matrix all come from the
same averaged profiled REML objective above.

BLMM then uses a local second-order approximation around the anchor. Let \(H\)
be the observed Hessian of \(\bar{\ell}\) at the anchor \(\bar{\phi}\). For
draw \(s\), BLMM computes a draw-specific score correction \(g_s\) from the
difference between the average profiled score and the draw-specific profiled
score at the anchor, and applies the local Newton update
...
full REML optimisation from scratch. Conditional on \(\phi_s\), BLMM then
solves the fixed effects exactly by generalized least squares:
...
with \(V_s = Z \Sigma_{\theta_s} Z^T + \sigma_s^2 I_N\) implied by the
updated covariance parameters. The approximation is therefore confined to the
repeated covariance optimisation step; the per-draw fixed-effects solve
remains exact conditional on the updated covariance model.
