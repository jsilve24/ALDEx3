...
Here is an example data analysis using the published [SR-MEM method](https://www.biorxiv.org/content/10.1101/2025.08.05.668734v1) (Scale Reliant Mixed Effects Models). Mixed effects modeling can be performed in ALDEx3 with the exact `lme4` and `nlme` interfaces, or with the approximate `blmm` engine. `blmm` uses a batched profiled mixed-model anchor fit plus draw-specific local covariance updates, while keeping the final fixed-effect solve exact conditional on the updated covariance. See the dedicated mixed-effects vignette for algorithmic details, validation, and guidance on when the approximation is appropriate. 
...
To use the approximate engine instead, change `method="lme4"` to
`method="blmm"`. The model formula is unchanged; only the covariance
optimisation step is approximated.
